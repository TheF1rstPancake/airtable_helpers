import { RecordData, RecordField } from './types/Record'
...
export function splitStudioSpaces(val: string, studios: string[]): string[] {
	...
	val.split('').forEach((char, index) => {
		...
		if (char === ' ') {
			if (raw.length === 0) {
				// Hit a space & lest studio did not have an area, Ex: A B O:P
	...
	return ranges
		.map((r) => {
			if (r.includes('-')) {
				...
				if (start > end)
					throw new Error(
						`Invalid studio area range: Start ${start}, End ${end}`
					)
				if (!areas.includes(start))
					throw new Error(`Studio does not have area ${start}`)
				if (!areas.includes(end))
					throw new Error(`Studio does not have area ${end}`)
				return areas
					.slice(areas.indexOf(start), areas.indexOf(end) + 1)
					.map(String)
			} else {
				return r
			}
...
export function splitGroupedStudioSpaces<T extends RecordField>(
	...
	grouped.map((item) => {
		...
		if (item.includes(':')) {
			/** Spaces with Complex Areas */
			//const id = m.workspaceAbbreviation.fieldId
			const [space, area] = item.split(':')
			let studio = studios.find(
				(studio) =>
					studio.fields.workspaceAbbreviation &&
					studio.fields.workspaceAbbreviation === space
			)
			if (!studio)
				studio = studios.find(
					(studio) =>
			...
			const studioSpaces: string[] = (studio.fields.workspaceAreas as string)
				.split(',')
...
export function validateStudioSpaces<T extends RecordField>(
	...
	const issues = userStudios.filter((uStudio) => {
		...
		if (found.fields.workspaceAreas) {
			...
			if (Array.isArray(uStudio.areas)) {
				// Standard areas
				...
				const invalid = subSpace.areas.find(
					(area) => !studioAreas.includes(subSpace.subSpace + area)
				)
				if (invalid && invalid.length) return true
			}
		}
...
export function hasStudioConflicts(
...
export function getStandardLiveShowSpacesAsString<T extends RecordField>(
	...
	return records
		.filter((rec) => rec.fields.isStudioSpace)
		.map((studio) => {
			...
			if (areas && /([A-Za-z])\w+/.test(areas)) {
				// Complex Sub Spaces
				let subAreas = areas.split(',').map((area) => area.substring(0, 1))
				return subAreas
					.filter((area, index) => subAreas.indexOf(area) === index)
					.map((area) => studioName + ':' + area)
					.join(' ')
			} else {
				return studioName
			}
	...
	if (studio0.areas) {
		...
		if (Array.isArray(studio0.areas)) {
			...
			mergedAreas = mergedAreas
				.filter((area, index) => area && mergedAreas.indexOf(area) === index)
				.sort()
			return {
				space: studio0.space,
				areas: mergedAreas,
			...
			if (!areas0.areas.length || !areas1.areas.length)
				return {
					space: studio0.space,
...
export function RemoveAreas(
	...
	if (studio0.areas) {
		...
		if (Array.isArray(studio0.areas)) {
			...
			/** Handle Complex Sub Spaces / Areas */
			...
			if (studio1.areas) {
				...
				valid = area1.areas.length
					...
					: areas
							.filter((area) => !area0.areas.includes(area.substring(1)))
...
function stringifyArea(area: string, self: string[], allAreas: string[]): string {
...
function stringifyComplexAreas<T extends RecordField>(
	...
	const allAreas = (subSpaceRecord.fields.workspaceAreas as string)
		.split(',')
...
export function stringifyStudios<T extends RecordField>(
	...
	return studios
		.map((studio, i) => {
			...
			const studioRecord = studioRecords.find(
				(rec) => rec.fields.workspace === studio.space
			)
